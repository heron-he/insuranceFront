...
function OverviewAreaLine({ data, loading, name = '总内容量', color = '#4080FF' }: { data: any[]; loading: boolean; name?: string; color?: string }) {
    return (
        <Spin loading={loading} style={{ width: '100%' }}>
            <Chart scale={{ value: { min: 0 } }} padding={[10, 20, 50, 40]} autoFit height={300} data={data} className={'chart-wrapper'}>
                ...
                >
                    {(title, items) => {
                        return <CustomTooltip title={title} data={items} color={color} name={name} formatter={(value) => Number(value).toLocaleString()} />
                    }}
                </Tooltip>
            </Chart>
        </Spin>
